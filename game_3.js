...
class Player {
    ...
    set_coll_balls_along_ring(player_rad, ring_ind, dist) {
        var hole_check = overlap_hole_region(player_rad, maze.rings[ring_ind].holes_coords, true);
...
function overlap_children(node) {
    ...
    var parent_hcoords = holes_from_coords(maze.rings[parent_layer_ind].holes_coords);
    var rad = parent_hcoords[parent_id].start;
    // 2. get children bounds
    var h_coords_children_layer = holes_from_coords(maze.rings[parent_layer_ind + 1].holes_coords);
    ...
    // debug
    if (!test) {
        maze.rings[parent_layer_ind].overlaps = [rad];
        maze.rings[parent_layer_ind + 1].debug_holes_inner = [bounds];
    }
    return test;
    // }
}
function resolve_overlapping_holes() {
    ...
    maze.rings[hole_problem_level].set_holes(coords);
    var this_hcoords = maze.rings[hole_problem_level - 1].holes_coords;
    var inner_hcoords = maze.rings[hole_problem_level].holes_coords;
    if (!overlap_holes(this_hcoords, inner_hcoords, hole_problem_level - 1)) {
        ...
        for (let index = 0; index < network.layers[hole_problem_level - 1].length; index++) {
            ...
            if (hole_problem_level < n_rings - 1 && overlap_children(parent_node)) { // not last ring
                ...
                hole_problem_level++;
                maze.rings[parent_node.ind].overlaps = [];
                maze.rings[parent_node.ind + 1].debug_holes_inner = [];
                break;
            }
        }
    }
...
function set_walls_one_layer(index) {
    var tol = (maze.rings[index].diameter/2)*0.00005;
    for (let index2 = 0; index2 < network.layers[index - 1].length; index2++) {
        ...
        var max_id = node.children[node.children.length-1].id;
        // then convert holes coords to start/end of actual holes
        var hcoords_parent = holes_from_coords(maze.rings[index - 1].holes_coords);
        var h_coords_children_layer = holes_from_coords(maze.rings[index].holes_coords);
        var min = h_coords_children_layer[min_id].start;
        var max = h_coords_children_layer[max_id].end;
        // assuming parent start is always between bounds of children
        maze.rings[index].add_wall((min - tol)%(2*Math.PI));
    }
}
...
function remove_virtuals_debug_layer(index) {
    ...
    // convert to holes coords
    var hcoords = holes_from_coords(maze.rings[index].holes_coords);
    ...
    // debug
    maze.rings[index].debug_holes = non_virtual_holes;
    maze.rings[index].debug_holes_inner = virtual_holes;
...
function resolve_overlaps_last_layer(parent_node) {
    ...
    if (!overlap_holes(this_hcoords, inner_hcoords, hole_problem_level - 1)) {
        ...
        if (overlap_children(parent_node)) {
            ...
            maze.rings[parent_node.ind + 1].debug_holes_inner = [this_hcoords[parent_node.id]];
        }
