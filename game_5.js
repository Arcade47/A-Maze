...
function add_values() {
    ...
    ring_spacing = 40;      // spacing of maze rings in px
    ring_thickness = 8;    // diameter of maze walls in px
...
class Player {
    ...
    set_closest_collision_balls() {
        ...
        // get two distances from center (two maze.rings)
        var dists = [];
...
class MazeNetworkRandom {
    ...
    render() {
        ...
        for (let index = 0; index < this.visited_cells.length; index++) {
            ...
            // connect with lines
            if (index > 0) {
                var pos1 = this.visited_cells[index - 1].get_pos();
                var pos2 = visited_cell.get_pos();
...
class MazeRandom {
    ...
    init_walls() {
        ...
        for (let index = 1; index < this.rings.length; index++) {
            var n_walls = this.network.rings[index - 1].length;
    ...
    dig() {
        ...
        if (rri != cri) {
            ...
            var ring = network.rings[ri];
            var n = ring.length;
            var c = this.rings[ri].get_c();
            var hcoords = equidistant_holes_coords(n, c, ring_spacing, ring_thickness);
            // identify ind in ring
            var ir = Math.min(rir, cir);
    ...
    update() {
        if (network.visited_cells.length == network.n_cells) {
            ...
            for (let index = 0; index < this.rings.length; index++) {
                const ring = this.rings[index];
                console.log(ring.holes_coords);
                var line_seg_coords = coords_from_holes(ring.holes_coords, false, true);
                console.log(line_seg_coords);
            }
        } else {
            this.dig();
        }
...
class Ring {
    ...
    add_hcoord_to_hcoords(hcoord) {
        ...
        this.edge_ball_coords.push(hcoord.start);
        this.edge_ball_coords.push(hcoord.end);
    }
...
function draw() {
    ...
    maze.render();
    // debug
    draw_debug_text(player.current_ring_ind);
}
